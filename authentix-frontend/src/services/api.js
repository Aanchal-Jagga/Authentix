...
export const detectGazeVideo = async (file) => {
  const formData = new FormData()
  formData.append('file', file)
  const { data } = await api.post('/api/gaze/detect-gaze-video-realtime', formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  })
  return data
}
...
// ── Forensic Analysis ──
export const forensicAnalyze = async (file) => {
  const formData = new FormData()
  formData.append('file', file)
  const { data } = await api.post('/forensic/analyze-image', formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  })
  return data
}
